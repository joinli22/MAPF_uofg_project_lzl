...
parser.add_argument('--workers', type=int, default=None, help='Number of parallel ECBS workers (processes). Default: use all CPUs')
...
class CasesGen:
    def __init__(self, config):
        ...
        self.PROCESS_NUMBER = config.workers or os.cpu_count() or 1
        print(f"Using {self.PROCESS_NUMBER} workers (processes)")
...
(array([      23,       26,       29, ..., 99999945, 99999950, 99999975]), array([1, 1, 1, ..., 1, 1, 1]))
    ...
    def setup_CasePool(self):
        ...
        # 按需求算出来“需要多少张地图”
        num_Env_need = int(round(num_data_exceed / num_cases_PEnv))
        # 实际可用地图数
        num_Env_have = len(self.list_path_loadSourceMap)
        # 取最小，避免 index out of range
        num_Env = min(num_Env_need, num_Env_have)

        if num_Env < num_Env_need:
            print(f"[WARN] maps available = {num_Env_have}, but need = {num_Env_need}. "
                  f"Will only generate {num_Env * num_cases_PEnv} cases.")
